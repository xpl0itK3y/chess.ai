...
function testFoolsMateCheckmate(): boolean {
            ...
            if (cell.figure && cell.figure.color === Colors.WHITE) {
                for (let ti = 0; ti < 8; ti++) {
                    for (let tj = 0; tj < 8; tj++) {
                        ...
                        if (cell.figure.canMove(target) && board.isMoveLegal(cell, target)) {
                            legalMoves.push(`  -> ${cell.figure.name} (${i},${j}) -> (${ti},${tj})`);
                        }
                    }
                }
            }
    ...
    return testResult('Спёртый мат определяется правильно', isCheck === true && isCheckmate === true);
}
...
function testBackRankMate(): boolean {
    ...
    return testResult('Мат на последней горизонтали', isCheck === true && isCheckmate === true);
}
...
function testCheckButNotMate(): boolean {
    ...
    return testResult('Шах, но не мат - король может уйти', isCheck === true && isCheckmate === false);
}
...
function testCheckCanBeBlocked(): boolean {
    ...
    return testResult('Шах можно перекрыть слоном', isCheck === true && isCheckmate === false);
}
...
function testStalemate(): boolean {
    ...
    return testResult('Пат определяется правильно', isCheck === false && isStalemate === true);
}
...
function testPawnCanCaptureAttacker(): boolean {
    ...
    return testResult('Пешка может срубить атакующего - не мат',
        isCheck === true && canCapture === true && isLegal === true && isCheckmate === false);
}
...
function runAllTests() {
    console.log('╔════════════════════════════════════════╗');
    console.log('║   ТЕСТЫ ШАХМАТНОЙ ЛОГИКИ               ║');
    console.log('╚════════════════════════════════════════╝');
